...
public class TreeMasterDetailViewConfigurator extends TreeViewConfigurator {
	...
	private Image taskImage =Activator.getImageDescriptor("/icons/task.jpg").createImage();
	private Image customerImage =Activator.getImageDescriptor("/icons/customer.jpg").createImage();
	...
	public EReference getListFeature() {
		return LibraryPackage.Literals.LIBRARY__LIST_BOOK;
	}
	
	@Override
	public IEMFListProperty getListProperty() {
		return EMFProperties.multiList(
				LibraryPackage.Literals.LIBRARY__LIST_BOOK,
				LibraryPackage.Literals.BOOK__AUTHOR);
	}
	
	
	public boolean hasChildren(Object element){
		return (element instanceof Library &&((Library) element).getListBook().size() > 0)
			||(element instanceof Book && ((Book) element).getAuthor().size() > 0);
	}
